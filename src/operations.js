...
export default function Operation() {
  ...
  const [assertOne, setAssertOne] = useState([
    {name: "Mudflap", isSelected: false},
    {name: "Ratchet", isSelected: false} ,
    {name: "Sideswipe", isSelected: false},
    {name: "Skids", isSelected: false}
  ])
  const [assertTwo, setAssertTwo] = useState([
    {name: "Charlotte Brontë", isSelected: false},
    {name: "Emily Brontë", isSelected: false},
    {name: "Jane Austen", isSelected: false},
    {name: "Louisa May Alcott", isSelected: false}
  ])
  const [assertThree, setAssertThree] = useState([
    {name: "True", isSelected: false},
    {name: "False", isSelected: false}
  ])
  // const [correctOne, setCorrectOne] = useState(null)
  function handleCorrectOne(index){
    setAssertOne(prev => prev.map((item, x) => ({...item, isSelected: x === index})))
  }
  // const [correctTwo, setCorrectTwo] = useState(null)
  function handleCorrectTwo(index){
    setAssertTwo(prev => prev.map((item, y)=> ({...item, isSelected: index === y})))
  }
  // const [correctThree, setCorrectThree] = useState(null)
    ...
    <section className="flex justify-center pt-20">
      <div className="w-6/12">
        ...
        <div className="mt-8">
          ...
          <div>
            <p className={`${showResult ? "" : 'hidden'} text-center font-bold text-blue-500`}>{`Your result is ${result.length} / 3`}</p>
          </div>
          <div className="mt-20 flex justify-center">
            {showResult ? <button onClick={()=> window.location.reload()} className="bg-blues p-4 text-white font-bold rounded-lg shadow-2xl">Next challenge</button>:<button onClick={()=>{setAnswer(handleAnswers());handleResultOne();handleResultTwo();handleResultThree(); setShowResult(true)}} className="bg-blues p-4 text-white font-bold rounded-lg shadow-2xl">Result</button>
          }
          </div>
        </div>
      </div>
